...
{
	// not yet implemented
	if(DEBUG==1) printk("ylc enqueue_task_weighted_rr\n");
	struct weighted_rr_rq  *weighted_rr_rq = &(rq->weighted_rr);
	list_add_tail(&(p->weighted_rr_list_item), &(weighted_rr_rq->queue));
	weighted_rr_rq->nr_running++;
}
...
static void dequeue_task_weighted_rr(struct rq *rq, struct task_struct *p, int sleep)
{
	...
	if(DEBUG==1) printk("ylc-debug: dequeue_task_weighted_rr\n");
...
{
	// not yet implemented
	if(DEBUG==1) printk("ylc-debug: yield_task_weighted_rr\n");
	struct task_struct *curr = rq->curr;
...
static struct task_struct *pick_next_task_weighted_rr(struct rq *rq)
{
	...
	// not yet implemented
	if(DEBUG==1) printk("ylc-debug: pick_next_task_weighted_rr\n");
	weighted_rr_rq = &(rq->weighted_rr);
	
	if(weighted_rr_rq->nr_running>0){
...
static void task_tick_weighted_rr(struct rq *rq, struct task_struct *p,int queued)
{
	...
	// not yet implemented
	if(DEBUG==1) printk("ylc-debug: task_tick_weighted_rr\n");
	weighted_rr_rq = &(rq->weighted_rr);
	curr = p;
