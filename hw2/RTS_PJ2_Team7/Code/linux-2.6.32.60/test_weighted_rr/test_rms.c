...
#define SYS_sjf_getquantum 339
#define SYS_sjf_setquantum 340
#define SYS_rms_getquantum 341
#define SYS_rms_setquantum 342
...
int main(int argc, char *argv[])
{
	...
	if(sched_policy == SCHED_SJF)
	{
		...
		old_quantum = syscall (SYS_sjf_getquantum);
		syscall (SYS_sjf_setquantum, quantum);
	}
	...
	for (i = 0; i < num_threads; i++)
	{
		...
		if(i == num_threads-1) execTime = quantum;
		printf("%c job exeTime=%d\n",targs->mychar,execTime);
		if(i == num_threads-1) usleep(10);
		if(quantum <= i)
			printf("TIme quantum too small\n");
		else 
			syscall (SYS_sjf_setquantum, execTime);
	...
	//+ reset time quantum
	syscall (SYS_sjf_setquantum, old_quantum);
