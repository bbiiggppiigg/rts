...
//+ RTS Proj2: sjf
static inline int sjf_policy(int policy)
{
	if(unlikely(policy == SCHED_SJF))
		return 1;
	return 0;
}

//+ RTS Proj2: sjf
static inline int task_has_sjf_policy(struct task_struct *p)
{
	return sjf_policy(p->policy);
}
//+ RTS Proj2: rms
static inline int rms_policy(int policy)
{
	if(unlikely(policy == SCHED_RMS))
		return 1;
	return 0;
}

//+ RTS Proj2: rms
static inline int task_has_rms_policy(struct task_struct *p)
{
	return rms_policy(p->policy);
}
...
struct sjf_rq {
	struct list_head queue;
	unsigned long nr_running;
	struct list_head *sjf_load_balance_head, *sjf_load_balance_curr;
};
struct rms_rq {
	struct list_head queue;
	unsigned long nr_running;
	struct list_head *rms_load_balance_head, *rms_load_balance_curr;
};
...
int sjf_time_slice = DEF_TIMESLICE / 1000;
int rms_time_slice = DEF_TIMESLICE / 1000;
...
static void __sched_fork(struct task_struct *p)
{
	...
	INIT_LIST_HEAD(&p->sjf_list_item);
	INIT_LIST_HEAD(&p->rms_list_item);
	
	p->sjf_time_slice = sjf_time_slice;
	p->rms_time_slice = rms_time_slice;
...
void sched_fork(struct task_struct *p, int clone_flags)
{
	...
	//+ RTS Proj2: weighted_rr
	if (!rt_prio(p->prio) && (p->sched_class != &weighted_rr_sched_class) && (p->sched_class != &sjf_sched_class) && (p->sched_class != &rms_sched_class))
...
void rt_mutex_setprio(struct task_struct *p, int prio)
{
	...
	if(prev_class == &sjf_sched_class)
			p->sched_class = &sjf_sched_class;
	if(prev_class == &rms_sched_class)
...
recheck:
	...
	if (policy < 0) {
		...
		policy != SCHED_IDLE && policy != SCHED_WEIGHTED_RR &&
		policy != SCHED_SJF && policy != SCHED_RMS) //+ RTS Proj2: weighted_rr
			return -EINVAL;
	}
	...
	printk("before set current task");
	if (running)
		p->sched_class->set_curr_task(rq);
	if (on_rq) {
		activate_task(rq, p, 0);
...
//+ RTS Proj2: sjf
SYSCALL_DEFINE0(sched_sjf_getquantum)
{
	return sjf_time_slice;
}

//+ RTS Proj2: sjf
SYSCALL_DEFINE1(sched_sjf_setquantum, unsigned int, quantum)
{
	sjf_time_slice = quantum;
	return sjf_time_slice;
}
//+ RTS Proj2: rms
SYSCALL_DEFINE0(sched_rms_getquantum)
{
	return rms_time_slice;
}

//+ RTS Proj2: rms
SYSCALL_DEFINE1(sched_rms_setquantum, unsigned int, quantum)
{
	rms_time_slice = quantum;
	return rms_time_slice;
}
...
static void init_sjf_rq(struct sjf_rq * sjf_rq){
	INIT_LIST_HEAD(&sjf_rq->queue);
}

static void init_rms_rq(struct rms_rq * rms_rq){
	INIT_LIST_HEAD(&rms_rq->queue);
}
